...
import tinyzero.verl.utils.hdfs_io as hdfs_io
import tinyzero.verl.utils.torch_functional as verl_F
from omegaconf import DictConfig, open_dict
from tinyzero.verl import DataProto
from tinyzero.verl.single_controller.base import Worker
from tinyzero.verl.single_controller.base.decorator import register, Dispatch
from tinyzero.verl.utils import hf_tokenizer
from tinyzero.verl.utils.debug import log_gpu_memory_usage
from tinyzero.verl.utils.fs import copy_local_path_from_hdfs
from tinyzero.verl.utils.fsdp_utils import get_fsdp_wrap_policy, offload_fsdp_grad, init_fn, get_init_weight_context_manager
from tinyzero.verl.utils.fsdp_utils import offload_fsdp_optimizer, offload_fsdp_param_and_grad, load_fsdp_optimizer, \
    load_fsdp_param_and_grad
from tinyzero.verl.utils.import_utils import import_external_libs
from tinyzero.verl.utils.model import compute_position_id_with_mask
from tinyzero.verl.utils.flops_counter import FlopsCounter
from tinyzero.verl.workers.sharding_manager.fsdp_ulysses import FSDPUlyssesShardingManager
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        from tinyzero.verl.utils.model import print_model_size, update_model_config
        from tinyzero.verl.utils.torch_dtypes import PrecisionType
        ...
        if use_remove_padding:
            from tinyzero.verl.models.registry import check_model_support_rmpad
            check_model_support_rmpad(actor_model_config.model_type)

        if use_remove_padding and self.ulysses_sequence_parallel_size > 1:
            from tinyzero.verl.models.transformers.monkey_patch import apply_monkey_patch
        ...
        if self._is_actor:
            from tinyzero.verl.utils.torch_functional import get_constant_schedule_with_warmup
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            from tinyzero.verl.workers.rollout import HFRollout
            from tinyzero.verl.workers.sharding_manager import BaseShardingManager
            rollout = HFRollout(module=self.actor_module_fsdp, config=self.config.rollout)
            rollout_sharding_manager = BaseShardingManager()
            # TODO: a sharding manager that do nothing?
        elif self.config.rollout.name == 'vllm':
            from tinyzero.verl.workers.rollout.vllm_rollout import vLLMRollout
            from tinyzero.verl.workers.sharding_manager import FSDPVLLMShardingManager
    ...
    def init_model(self):
        from tinyzero.verl.workers.actor import DataParallelPPOActor
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        from tinyzero.verl.utils.model import LambdaLayer, print_model_size, squeeze
        from tinyzero.verl.utils.torch_dtypes import PrecisionType
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP, ShardingStrategy, MixedPrecision
        from torch import optim
    ...
    def init_model(self):
        ...
        from tinyzero.verl.workers.critic import DataParallelPPOCritic
...
class RewardModelWorker(Worker):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        from tinyzero.verl.utils.ulysses import ulysses_pad_and_slice_inputs, gather_outpus_and_unpad
    ...
    def compute_rm_score(self, data: DataProto):
        import itertools
        from tinyzero.verl.utils.seqlen_balancing import rearrange_micro_batches, get_reverse_idx
        data = data.to('cuda')
