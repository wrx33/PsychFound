...
def get_fewshot_input_path(args):
    ...
    inp_pth = os.path.join('data', f'fewshot1', f'CMB-Exam-{cot_or_a}-{args.model_id}.json')
...
def main(args):
    ...
    dataloader_iterator = islice(dataloader_iterator, args.start - 1, None)
    ...
    for batch_idx, batch in enumerate(dataloader_iterator, start=args.start):
